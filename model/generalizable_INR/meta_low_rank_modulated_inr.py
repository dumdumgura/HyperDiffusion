...
class MetaLowRankModulatedINR(TransINR):
    ...
    def decode_with_modulation_factors(self, modulation_factors_dict,overfit=False,type='occ', res = 256, level= 0.07):
        r"""Inference function on Hyponet, modulated via given modulation factors."""
        #coord = self.sample_coord_input(xs) if coord is None else coord
        ...
        meshes = create_meshes(
                self.hyponet, modulation_params_dict, level=level, N=res,overfit=overfit,type=type
                )
    ...
    def overfit_one_shape(self, coord=None, type='occ', res=256, level=0.07):
        modulation_factors_dict = self.get_init_modulation_factors_overfit()
        # convert modulation factors into modulation params

        if coord is None:
            visuals = self.decode_with_modulation_factors(modulation_factors_dict, overfit=True,type=type, res=res, level=level)
            return visuals
        else:
